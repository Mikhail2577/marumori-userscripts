...
// @license      WTFPL
// @downloadURL https://raw.githubusercontent.com/Mikhail2577/marumori-userscripts/main/even-more-gamified/dist/marumori_even_more_gamified.user.js
// @updateURL https://raw.githubusercontent.com/Mikhail2577/marumori-userscripts/main/even-more-gamified/dist/marumori_even_more_gamified.meta.js
// ==/UserScript==
...
(() => {
  ...
  function createFrameCadenceGate({ intervalMs = 1e3 / 60, earlyToleranceMs = 0.5 } = {}) {
  ...
  function createShootingStarSystem({
    ...
    function trigger() {
      if (isLiteMode2() || !settings2.visualsEnabled || prefersReducedMotion2() || isAnswerResolved2() || !hasShootingStars() || width <= 0 || height <= 0) {
        return false;
      }
      stars.push({
    ...
    function draw(ctx, frameScale) {
      ...
      compactInPlace(stars, (star) => {
        ...
        return !(outsideLeft && star.vx <= 0 || outsideRight && star.vx >= 0 || outsideTop && star.vy <= 0 || outsideBottom && star.vy >= 0);
      });
  ...
  function createCanvasBackgroundController({
    ...
    function resumeArcadeBackdrop() {
      ...
      if (!settings2.visualsEnabled || !hasCanvasBackdrop() || !isSessionActive() || document2.hidden || reducedMotion) {
        pauseArcadeBackdrop();
        return false;
      }
      if (!document2.getElementById("mm-starfield")) {
        buildStarfield();
        return Boolean(document2.getElementById("mm-starfield"));
      }
      return resumeStarfield?.() === true;
    }
    function syncReducedMotion() {
      const nextReducedMotionState = prefersReducedMotion2();
      pauseArcadeBackdrop();
      if (nextReducedMotionState) {
        if (isSessionActive() && !document2.hidden) renderStarfieldOnce?.();
        return false;
      }
    ...
    function buildStarfield() {
      ...
      if (document2.getElementById("mm-starfield") || starResizeHandler || starVisibilityHandler) {
        stopArcadeBackdrop();
      }
      ...
      function scheduleNextFrame() {
        if (generation !== starGeneration || !isSessionActive() || prefersReducedMotion2() || isLiteMode2() && isStaticImageTheme || document2.hidden) {
          return;
        }
        if (isLiteMode2()) {
      ...
      function tick(now = performanceNow(), force = false, schedule = true) {
        ...
        if (document2.hidden || !force && !isSessionActive()) return;
        if (!isLiteMode2() && !isMaxMode2()) {
          if (force) {
      ...
      renderStarfieldOnce = () => {
        if (generation !== starGeneration || document2.hidden) return false;
        tick(performanceNow(), true, false);
        return true;
      };
      resumeStarfield = () => {
        if (generation !== starGeneration || starRaf !== null || starFrameTimer !== null || !isSessionActive() || document2.hidden || prefersReducedMotion2()) {
          return false;
        }
        starActivityGeneration++;
        lastRenderAt = 0;
      ...
      starResizeHandler = debounce(() => {
        resize();
        renderer.init();
        if (prefersReducedMotion2() || isLiteMode2() && isStaticImageTheme || !isSessionActive()) {
  ...
  function createPanelElement(document2, settings2, getMusicModeLabel, getThemeLabel, panelExtension) {
  ...
  var STAT_TONES = /* @__PURE__ */ new Set(["gold", "green", "cyan", "orange", "pink"]);
  function createDialogElement(document2) {
    const overlay = document2.createElement("div");
  ...
  function createSummaryDialogController({ document: document2, getFallbackFocus = () => null } = {}) {
    if (!document2?.body) throw new TypeError("Summary dialog requires a document body");
    if (typeof getFallbackFocus !== "function") {
      throw new TypeError("Summary dialog fallback focus must be a function");
    }
    const element = createDialogElement(document2);
    ...
    function installDocumentListeners() {
      ...
      document2.addEventListener("focusin", handleDocumentFocusin, true);
    }
    function removeDocumentListeners() {
      if (!documentListenersInstalled) return;
      documentListenersInstalled = false;
      document2.removeEventListener("keydown", handleDocumentKeydown, true);
      document2.removeEventListener("focusin", handleDocumentFocusin, true);
    }
    function render({ grade = "", gradeColor = "", stats = [] } = {}) {
      ...
      const cells = stats.map(({ label = "", value = "", tone = "" } = {}) => {
        ...
        cell.append(document2.createTextNode(String(label)));
        const valueElement = document2.createElement("span");
    ...
    function captureAndDisableBackground() {
      backgroundSnapshot = [...document2.body.children].filter((candidate) => candidate !== element).map(snapshotBackgroundElement);
    ...
    function restoreFocus() {
      ...
      previousFocus = null;
      if (target && target !== document2.body && focusWithoutScrolling(target)) return;
  ...
  var summaryDialogController = null;
  ...
  var themePreviewFeature = createThemePreviewFeature({
  ...
  function injectStyles() {
    ...
    s.textContent = styles_default + THEME_PREVIEW_STYLES;
    ...
    summaryDialogController = createSummaryDialogController({
      ...
      getFallbackFocus() {
        ...
        return candidates.find(
          (candidate) => candidate?.isConnected && !candidate.hidden && !candidate.closest?.('[hidden], [inert], [aria-hidden="true"]')
        ) ?? null;
      }
    });
  ...
  var canvasBackgroundController = createCanvasBackgroundController({
    document,
    window,
    settings,
    ...
    prefersReducedMotion,
    isAnswerResolved,
    isSessionActive: () => initialized && lifecycle.sessionState === SESSION_STATES.ACTIVE && state.sessionActive
  });
  var {
